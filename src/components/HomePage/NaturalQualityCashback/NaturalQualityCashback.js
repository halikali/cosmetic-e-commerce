import React from "react";
import "./styles.scss";

const NaturalQualityCashback = () => {
  return (
    <section className="natural-quality-cashback-section margin-all">
      <div className="container">
        <div className="row">
          <div className="col-lg-4 padding">
            <div className="card">
              <i class="fas fa-leaf icon"></i>
              <div className="card-body">
                <h5 className="card-title">ALL NATURAL</h5>
                <p className="card-text px-3 my-3">
                  For county now sister engage had season better had waited.
                  Occasional mrs interested far expression.
                </p>
              </div>
            </div>
          </div>
          <div className="col-lg-4 padding">
            <div className="card">
              <i class="fas fa-crown icon"></i>
              <div className="card-body">
                <h5 className="card-title">TOP QUALITY</h5>
                <p className="card-text px-3 my-3">
                  Service get met adapted matters offence for. Principles man
                  any insipidity age you simplicity.
                </p>
              </div>
            </div>
          </div>
          <div className="col-lg-4 padding">
            <div className="card">
              <i class="fas fa-coins icon"></i>
              <div className="card-body">
                <h5 className="card-title">10% CASHBACK</h5>
                <p className="card-text px-3 my-3">
                  Use securing confined his shutters. Delightful as he it
                  acceptance an solicitude discretion reasonably.
                </p>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
};

export default NaturalQualityCashback;
